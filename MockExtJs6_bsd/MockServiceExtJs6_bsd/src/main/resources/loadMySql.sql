...
delete FROM Authorities; -- must delete before Users
delete FROM Users;
-- delete FROM ids_employee_profile - Cannot delete or update a parent row: a foreign key constraint fails 
-- (`mockstub`.`ids_security`, CONSTRAINT `FK_dma77agpb7vugif6y9td04g3y` FOREIGN KEY (`ids_user_id`) REFERENCES `ids_employee_profile` (`employee_id`))	0.202 sec

-- delete FROM ids_security	- Cannot delete or update a parent row: a foreign key constraint fails 
-- (`mockstub`.`ids_manager`, CONSTRAINT `FK_6wefrbuvq97pv9oqi051hgvph` FOREIGN KEY (`alt_salesrep_cd`) REFERENCES `ids_security` (`alt_salesrep_cd`))	0.093 sec
